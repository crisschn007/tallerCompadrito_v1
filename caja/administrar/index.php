...
// Obtener la última caja (abierta o cerrada)
$queryCaja = $pdo->query("SELECT * FROM Caja ORDER BY id_caja DESC LIMIT 1");
$caja = $queryCaja->fetch(PDO::FETCH_ASSOC);

$id_caja = $caja ? $caja['id_caja'] : null;
$estadoCaja = $caja ? strtoupper($caja['estado']) : 'SIN CAJA';

// Variables iniciales
$montoInicial = $montoActual = $ingresos = $egresos = $devoluciones = $prestamos = $gastos = 0.00;
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary">
  ...
  <script>
    async function actualizarEstadoCaja() {
      ...
      try {
        const res = await fetch('../../app/controllers/caja/estado_caja.php');
